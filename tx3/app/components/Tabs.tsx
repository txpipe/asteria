...
import { useState, useMemo, Children, isValidElement, type PropsWithChildren, type ReactNode } from 'react';
        ...
        {tabs.map((tab, index) => (
          <button
            ...
            className={clsx(
              ...
              'border-transparent text-[#A0A0A0] hover:text-accent/50 hover:border-accent/50',
              'aria-selected:border-accent aria-selected:text-accent',
            )}
      ...
      <div className={clsx('flex flex-1', contentClassName)}>
        {tabs.map((tab, index) => (
          <div
            ...
            tabIndex={index}
            className="hidden data-selected:block w-full"
          >
            {tab.content}
          </div>
        ))}
